...
## Solution :
We have $Y\sim Ga(\theta,3)$, then
\begin{eqnarray*}
  \mathcal{L}(\theta;y) &\propto& \frac{1}{\Gamma(\theta)^n}3^{n\theta}\prod_{i=1}^ny_i^{\theta-1}\\
  \mathcal{l}(\theta;y) &=& -n\log\Gamma(\theta) + n\theta\log 3 + (\theta-1)\sum_{i=1}^{n}\log y_i \\
  &=& -n\log\Gamma(\theta) + n\theta\log 3 + (\theta-1)t_1\\
  \textit{with}\; t_1&=& \sum_{i=1}^{n}\log y_i
\end{eqnarray*}
Therefore 
\begin{eqnarray*}
  \mathcal{l}_*(\theta;y) &=& -n\psi(\theta) + n\log 3 + t_1 = 0\\
  \mathcal{l}_{(2)}(\theta;y) &=& n\frac{\Gamma'(\theta)-\Gamma''(\theta)\Gamma(\theta)}{\Gamma(\theta)^2}
\end{eqnarray*}
\newpage 
